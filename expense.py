...
# app.py — Personal Finance & Spending Analyzer (V16 - FACE DETECTOR LOGIN)
...
# === NEW ML/CV IMPORTS (REPLACING MEDIAPIPE HANDS) ===
import cv2
# Note: MediaPipe is still imported but not used in the transformer
...
class FaceDetectorTransformer(VideoTransformerBase):
    ...
    """
    def __init__(self):
        # NOTE: This path needs to be adjusted if the Haar Cascade file is not found
        # It's usually included with the OpenCV Python package
    ...
    def recv(self, frame):
        ...
        # Detect faces (scaled down by 1.1, minNeighbors=5)
        ...
        if self.face_detected_count > 0:
            ...
            cv2.putText(img, "FACE DETECTED", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2, cv2.LINE_AA)
            
            # Update the biometric vector (using size and position as the "code")
            # In a real app, this would be a secure embedding
            self.current_face_vector = f"{x},{y},{w},{h},{datetime.now().hour}"
        else:
            self.current_face_vector = ""

        # Draw a status text at the top left
...
# Since Haar Cascade only checks for face presence, we simplify the "3-step" to "Face Present"
VALID_USERS = {
...
# Simplified state for Face Detection (only check presence)
if "ml_login_step" not in st.session_state:
    st.session_state["ml_login_step"] = 1
# We only store one captured code now, as it's a simpler face authentication
...
def _login_view() -> None:
    ...
    # Create the webrtc streamer and get the video transformer
    ctx = webrtc_streamer(
        key="ml_webcam_input",
    ...
    # Safely retrieve data from the video processor
    ...
    with st.container():
        ...
        if st.button("Scan and Login", use_container_width=True):
            ...
            if face_detected_count == 0:
                ...
                # In a real system, you'd use a complex ML comparison (e.g., face embedding check)
                # Here, we use a simple success condition since Haar is not for unique identification.
    ...
    with st.container():
        ...
        if st.button("Enroll Face & Create User", key="enroll_button", use_container_width=True):
            ...
            if not new_user_id:
                ...
                # Add the new user with a simple placeholder face code
    ...
    # Display the current vector for debugging/illustration
...
with tab_dashboard:
    ...
    with tb6:
        if st.button("Logout", key="logout_1"):
            ...
            for k in ("auth_ok", "auth_user", "chat_history", "coin_rain_show", "coin_rain_start", "longest_streak_ever", "promise_text", "last_quote", "daily_data", "DB", "ml_login_step", "ml_face_code_live"):
