...
# User registration logic simplified: User ID is the key. Presence means registered for biometrics.
# Storing the wake word dynamically in session state.
VALID_USERS = { 
    "prakriti11": True,
    "ujjwal11": True,
    "aditya11": True,
...
if "user_wake_words" not in st.session_state:
    # Initialize with default wake word for initial users
    st.session_state["user_wake_words"] = {u: "hey machine" for u in VALID_USERS.keys()}

def _get_all_users(db: MiniDB) -> set:
    db_users = {t.user_id for t in db._tx.values()}
    return set(st.session_state["user_wake_words"].keys()) | db_users
...
def _login_view() -> None:
    ...
    <div class="navbar">
      <div style="display:flex;justify-content:space-between;align-items:center;width:100%">
        <div>
          <div class="nav-title">🔐 Finance Analyzer — USER LOGIN</div>
          <div class="nav-sub">Simple Access or Biometric Login via Face Detection</div>
        </div>
      </div>
    </div>
    """,
    ...
    # 1. SIMPLE LOGIN (Username only - or New User Creation)
    with st.container():
        st.markdown('<div class="login-card-inner" style="background:#f0f7ff">', unsafe_allow_html=True)
        st.subheader("1. Standard Login (Username Only)")
        st.info("Enter your existing username or a new one to create a profile.")

        u_simple = st.text_input("Username (e.g., prakriti11)", key="user_simple_text")
        
        if st.button("Continue to Dashboard", use_container_width=True, key="simple_login_button", type="primary"):
            user_id = u_simple.strip().lower()
            if not user_id:
                st.error("Username cannot be empty.")
            else:
                # If new user, initialize their wake word to the default
                if user_id not in st.session_state["user_wake_words"]:
                    st.session_state["user_wake_words"][user_id] = "hey machine"
                
                st.session_state["auth_ok"] = True
                st.session_state["auth_user"] = user_id
                st.session_state["chat_history"] = []
                st.success(f"🎉 **Standard Login Success!** Welcome, **{user_id}**.")
                st.rerun()

        st.markdown("</div>", unsafe_allow_html=True)

    # 2. BIOMETRIC LOGIN/ENROLLMENT (Face Scan + Wake Word)
    with st.container():
        st.markdown('<div class="login-card-inner" style="background:#fff7f0">', unsafe_allow_html=True)
        st.subheader("2. Biometric Enroll & Login")
        st.warning("Enrollment/Login requires camera access and a single face detection.")
        
        # Re-init webcam streamer
        ctx = webrtc_streamer(
            key="ml_webcam_input",
        ...
        u_biometric = st.text_input("Username for Biometric Login", key="user_biometric_text")
        
        # Determine the current/default wake word for display/recording
        is_enrolled = u_biometric.strip().lower() in st.session_state["user_wake_words"]
        current_wake_word = st.session_state["user_wake_words"].get(u_biometric.strip().lower(), "hey machine")

        col_ww, col_btn = st.columns([2, 1])

        with col_ww:
            if is_enrolled:
                st.info(f"Existing wake word: **{current_wake_word}**.")
                new_word = st.text_input(f"Change recorded wake word?", value=current_wake_word, key="change_wake_word")
                if new_word.strip().lower() != current_wake_word and new_word.strip():
                    st.session_state["user_wake_words"][u_biometric.strip().lower()] = new_word.strip().lower()
                    st.success(f"Wake word updated to **{new_word.strip().lower()}**.")
                    st.rerun()
            else:
                new_word = st.text_input("New Wake Command to Record", value="hey machine", key="record_new_wake_word")

        # Use the most recent input for the enrollment/login logic
        recorded_word = (new_word if is_enrolled else new_word).strip().lower()

        if face_detected_count > 0:
            st.success(f"Face detected! Use **{recorded_word}** as your wake word.")
        else:
            st.error("No face detected. Please look at the camera.")

        # --- Enroll Logic ---
        if not is_enrolled:
            if st.button("Enroll Face & Record Wake Word (New User)", key="enroll_button_2", use_container_width=True):
                new_user_id = u_biometric.strip().lower()
                
                if not new_user_id:
                    st.error("Username cannot be empty.")
                elif new_user_id in st.session_state["user_wake_words"]:
                    st.info(f"User **{new_user_id}** is already enrolled. Use 'Biometric Login' below.")
                elif face_detected_count != 1:
                    st.error("❌ **Enrollment Error:** Must detect exactly one face.")
                elif not recorded_word:
                    st.error("Please enter a wake command to record.")
                else:
                    # SIMULATED ENROLLMENT & WAKE WORD RECORDING
                    st.session_state["user_wake_words"][new_user_id] = recorded_word
                    
                    st.session_state["auth_ok"] = True
                    st.session_state["auth_user"] = new_user_id
                    st.session_state["chat_history"] = []
                    st.success(f"🎉 **Biometric Enrolled!** Wake word set to **'{recorded_word}'**. Welcome, **{new_user_id}**.")
                    st.rerun()
        
        # --- Login Logic (Wake Word Equivalent) ---
        if st.button(f"Biometric Login (Hey Machine Method)", key="biometric_login_button", use_container_width=True):
            login_user_id = u_biometric.strip().lower()
            if login_user_id not in st.session_state["user_wake_words"]:
                st.error(f"❌ **Login Error:** User **{login_user_id}** is not enrolled for biometric login. Please enroll first.")
            elif face_detected_count != 1:
                st.error("❌ **Login Error:** Must detect exactly one face to log in.")
            else:
                # Successful login, set the wake word for the session
                st.session_state["auth_ok"] = True
                st.session_state["auth_user"] = login_user_id
                st.session_state["chat_history"] = []
                st.success(f"🎉 **Biometric Login Success!** Your wake word is **'{st.session_state['user_wake_words'][login_user_id]}'**.")
                st.rerun()
...
with tab_dashboard:
    ...
    with right_col:
        ...
        # Retrieve the user's wake word or default to "hey machine"
        current_user_wake_word = st.session_state.get("user_wake_words", {}).get(CURRENT_USER_ID, "hey machine")
        st.caption(f"Wake word **“{current_user_wake_word}”**: Hindi + English tips. Or press **Speak now**.")
        ...
        if enable_listener:
            # Using the dynamically set wake word
            smart_machine_listener(advice_hi, advice_en, wake_word=current_user_wake_word, order=order)
            st.info(f"Listening… say **{current_user_wake_word}**. Allow mic permission; keep this tab focused.")
