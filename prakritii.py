...
# ============================== Login & User Management (OAuth INTEGRATION) ==============================

# Note: The original VALID_USERS dict and custom login/register logic were removed.

# Ensure auth_ok is set for the rest of the application logic to run
if "auth_ok" not in st.session_state:
    st.session_state["auth_ok"] = False
    st.session_state["auth_user"] = None

# --- AUTHENTICATION CHECK (Using Streamlit's Native OAuth) ---
if not st.user.is_logged_in:
    
    # Display specialized login screen
    st.markdown(
        """
        <style>
        /* Ensures login view is styled correctly */
        [data-testid="stAppViewContainer"] > .main {background-color: #0f1117;}
        .login-title {
            font-size: 32px; font-weight: 800; color: #ff79b0; text-align: center; margin-bottom: 20px;
            text-shadow: 0 0 10px rgba(255, 105, 180, 0.7);
        }
        .navbar { /* Ensure navbar style is displayed correctly in login view */
            position: relative; padding: 12px 18px; margin: 0 0 18px 0;
            border-radius: 14px; 
        ...
        <div class="navbar">
          <div style="display:flex;justify-content:space-between;align-items:center">
            <div>
              <div class="nav-title">🔐 Gmail Direct Verification Login</div>
              <div class="nav-sub">Sign in securely with your Google account (OAuth 2.0)</div>
            </div>
            <div class="coin-wrap">
    ...
    st.markdown("<div class='login-title'>Access Requires Gmail Verification</div>", unsafe_allow_html=True)

    if st.button("Sign In with Google", key="google_login_btn", use_container_width=True):
         # This triggers the OAuth redirect based on the config in .streamlit/secrets.toml
         st.login(provider="google") 
    
    st.stop() # Stop execution until successful login
    
else:
    # User is successfully authenticated via Google OAuth
    
    # 1. Update session state flags
    st.session_state["auth_ok"] = True
    
    # 2. Use the Google user's email as the unique identifier for data segregation
    st.session_state["auth_user"] = st.user.email or "oauth_user"
        
    # 3. Initialize chat history 
    if "chat_history" not in st.session_state:
         st.session_state["chat_history"] = []
         
    # st.success(f"Google Login successful for: **{st.session_state['auth_user']}**. Loading dashboard...")

# --- END OF LOGIN REPLACEMENT ---
...
with tab_dashboard:
    ...
    with tb6:
        if st.button("Logout", key="logout_1"):
            # Clear all relevant session state for a clean log out and trigger native logout
    ...
    with left:
        ...
        if tmp.shape[0] == 0:
            ...
            if plot_type.startswith("Donut Chart"):
                ...
                # Heatmap: Expense by category over time period
                heatmap_df = tmp[tmp["type"] == "expense"].groupby(["period", "category"])["amount"].sum().reset_index()
                if heatmap_df.empty:
                    fig = go.Figure().update_layout(title="No Expense Data for Heatmap", template="plotly_dark", height=520)
                else:
                    pivot_df = heatmap_df.pivot_table(index='category', columns='period', values='amount', fill_value=0)
                    fig = px.imshow(pivot_df,
                                     title=f"Expense Heatmap by Category ({group_period})",
                                     x=pivot_df.columns, y=pivot_df.index,
                                     color_continuous_scale=px.colors.sequential.Reds)
                    fig.update_xaxes(side="top")
                    fig.update_layout(xaxis_title="Time Period", yaxis_title="Category")
            else:
