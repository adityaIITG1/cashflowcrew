...
# app.py — Personal Finance & Spending Analyzer (V18 - CV2 REMOVED / SIMPLIFIED LOGIN)
...
# === ML/CV/OCR IMPORTS ===
# NOTE: cv2, mediapipe, and streamlit_webrtc are REMOVED for stability.
# The following classes and imports are now obsolete but kept for reference:
# import cv2 
...
# ============================== Authentication (Simplified Login) ==============================

# REMOVED FaceDetectorTransformer class entirely.
# We no longer rely on cv2 or webcam libraries for stability.

# ============================== API Keys and Configuration ==============================

# NOTE: Using a hardcoded key here for demonstration based on the previous request, 
# but best practice is to use st.secrets or os.environ.get for security.
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY") or "AIzaSyCW-sKyNFFkWz4hiysMWJR6Fyj342-GhkY"
TELEGRAM_BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN")
TELEGRAM_CHAT_ID = os.environ.get("TELEGRAM_CHAT_ID")
OPENWEATHER_API_KEY = os.environ.get("OPENWEATHER_API_KEY")

# Hardcoded keys (public-only)
...
# ============================== Utilities / FX / Sound ==============================
# ... (All utility functions remain the same: generate_placeholder_image, _img64, _pick_qr_path, etc.)
...
# ============================== Login & User Management (SIMPLIFIED) ==============================

# Simulated User database with static password/ID for stable login
VALID_USERS_AUTH = {
    "prakriti11": "pass123",  # User 1
    "ujjwal11": "pass123",    # User 2
    "aditya11": "pass123",    # User 3
    "guest": "pass123",       # Simple guest login
}

def _get_all_users(db: MiniDB) -> set:
    db_users = {t.user_id for t in db._tx.values()}
    return set(VALID_USERS_AUTH.keys()) | db_users

def _login_view() -> None:
    st.markdown(
        """
    <style>
    /* Reusing login styles for aesthetic, but core functionality is form-based */
    [data-testid="stAppViewContainer"] > .main {background-color: #0f1117;}
    .login-card-container { display: flex; justify-content: center; padding-top: 50px;}
    .login-card-inner {
        padding: 30px; border-radius: 16px; max-width: 450px;
    ...
    <div class="navbar">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div>
          <div class="nav-title">🔐 Finance Analyzer — SIMPLE LOGIN</div>
          <div class="nav-sub">Stable Login Mechanism (CV2 Dependency Removed)</div>
        </div>
      </div>
    </div>
    """,
    ...
    with st.container():
        ...
        st.subheader("User Login (Simplified)")
        
        with st.form("login_form"):
            u_select = st.selectbox("Select Username", options=all_users, key="user_select_login")
            p_input = st.text_input("Password (Default: pass123)", type="password", key="password_input")
            ...
            if login_button:
                ...
                if u_select in VALID_USERS_AUTH and VALID_USERS_AUTH[u_select] == p_input:
                    st.session_state["auth_ok"] = True
                    st.session_state["auth_user"] = u_select
                    st.session_state["chat_history"] = []
                    st.success(f"🎉 **Login Success!** Welcome, **{u_select}**.")
                    st.rerun()
                # Attempt to register a new user (simple registration)
                elif u_select not in VALID_USERS_AUTH and p_input:
                     st.info(f"User **{u_select}** is new. Click 'Register' below to create this user.")
                else:
        ...
        with st.form("register_form"):
            u_new = st.text_input("New Username (no spaces)", key="user_new_register")
            p_new = st.text_input("Set Password", type="password", key="password_new")
            register_button = st.form_submit_button("Register New User", use_container_width=True)
            
            if register_button:
                new_user_id = u_new.strip()
                if not new_user_id or not p_new:
                    st.error("Username and Password cannot be empty.")
                elif new_user_id in VALID_USERS_AUTH or new_user_id in DB._tx:
                    st.error(f"User **{new_user_id}** already exists.")
                else:
                    # Register the new user
                    VALID_USERS_AUTH[new_user_id] = p_new
                    st.session_state["auth_ok"] = True
                    st.session_state["auth_user"] = new_user_id
                    st.session_state["chat_history"] = []
                    st.success(f"🎉 **New User Registered!** Welcome, **{new_user_id}**.")
                    st.rerun()
...
# (CSS styles remain the same, ensuring pink glow theme is kept)
st.markdown(
    f"""
<style>
...
with tab_dashboard:
    ...
    with left_col:
        ...
        default_amount = ocr_data.get("amount", 100.0) if ocr_data else 100.0
        default_date = ocr_data.get("date", date.today().isoformat()) if ocr_data else date.today().isoformat()
        default_desc = ocr_data.get("merchant", "Manual Entry") if ocr_data else "Manual Entry"
        default_cat = ocr_data.get("category", "uncategorized") if ocr_data else "uncategorized"

        with st.form("manual_txn_form", clear_on_submit=True):
            txn_date = st.date_input("Date", value=pd.to_datetime(default_date).date())
            ...
            txn_cat = st.selectbox(
                "Category", options=all_cats, index=all_cats.index(default_cat) if default_cat in all_cats else 0
            )
